...
$p1F0143SpecApprovalTransitionHotfixEvidencePath = "docs/05-execution-logs/evidence/2026-07-18-p1-f0143-spec-approval-transition-hotfix.md"
$p1F0143SpecApprovalTransitionHotfixAuditPath = "docs/05-execution-logs/audits-reviews/2026-07-18-p1-f0143-spec-approval-transition-hotfix.md"
...
$p1MechanismBootstrapPreCommitCorrectionTreeSha256 = "9b5c22c2d2b3e06114a35181fb7cb9b55e33d26becca3b6e15fb681cf6c289ad"
$p1MechanismBootstrapPreCommitCorrectionBaseSha = "d0b71842657f8f4df7e72d5fa6514b94d20b2de4"
$p1MechanismBootstrapPreCommitCorrectionBranch = "codex/p1-mechanism-bootstrap-manual-recognition"
$p1MechanismBootstrapPreCommitCorrectionFiles = @(
...
$p1MechanismBootstrapManualContractFiles = @(
...
function Get-P1F0143SpecApprovalTransitionHotfixSnapshotFileText {
    param(
        [Parameter(Mandatory = $true)][string]$Root,
        [Parameter(Mandatory = $true)][ValidateSet("INDEX", "HEAD")][string]$Snapshot,
        [Parameter(Mandatory = $true)][string]$Path
    ...
    $normalizedPath = ConvertTo-NormalizedPath -Path $Path
    try {
        $snapshotPaths = if ($Snapshot -ceq "INDEX") {
            @(& git -C $Root ls-files --cached -- $normalizedPath)
        } else {
            @(& git -C $Root ls-tree -r --name-only HEAD -- $normalizedPath)
        }
        $snapshotInspectionExitCode = $LASTEXITCODE
    } catch {
        return ""
    }
    $normalizedSnapshotPaths = @($snapshotPaths | ForEach-Object { ConvertTo-NormalizedPath -Path $_ })
    if ($snapshotInspectionExitCode -ne 0 -or $normalizedSnapshotPaths.Count -ne 1 -or $normalizedSnapshotPaths[0] -cne $normalizedPath) { return "" }
    return Get-GitSnapshotFileText -Root $Root -Snapshot $Snapshot -Path $normalizedPath
}
...
function Test-P1F0143SpecApprovalTransitionHotfixAnchors {
    ...
    $branch = ((& git -C $Root branch --show-current) -join "").Trim()
    $normalizedCurrentTaskBlock = @($CurrentTaskBlock | Where-Object { $null -ne $_ })
    $queueTaskIdMatches = @([regex]::Matches(($normalizedCurrentTaskBlock -join "`n"), '(?m)^\s*-\s+id:\s*(\S+)\s*$'))
    $queueTaskId = if ($queueTaskIdMatches.Count -eq 1) { $queueTaskIdMatches[0].Groups[1].Value } else { "" }
    $taskStatus = if ($normalizedCurrentTaskBlock.Count -eq 0) { "" } else { Get-ScalarValue -Block $normalizedCurrentTaskBlock -Key "status" }
    $parentReference = "HEAD"

    if ($CurrentPhase -ceq "pre_commit") {
        if ($headSha -cne $p1F0143SpecApprovalTransitionHotfixBaseSha -or $branch -cne $p1F0143SpecApprovalTransitionHotfixBranch) {
            Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_CONTEXT_INVALID pre_commit"
        }
        & git -C $Root diff --quiet
        $unstagedTrackedExitCode = $LASTEXITCODE
        $untrackedFiles = @(& git -C $Root ls-files --others --exclude-standard)
        if ($unstagedTrackedExitCode -ne 0 -or $LASTEXITCODE -ne 0 -or $untrackedFiles.Count -gt 0) {
            Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_PARTIAL_STAGE_INVALID"
        }
    } else {
        $headParents = @(((& git -C $Root rev-list --parents -n 1 HEAD) -join "").Trim() -split '\s+')
        $originMasterSha = ((& git -C $Root rev-parse origin/master) -join "").Trim()
        if ($headParents.Count -ne 2 -or $headParents[1] -cne $p1F0143SpecApprovalTransitionHotfixBaseSha -or $originMasterSha -cne $p1F0143SpecApprovalTransitionHotfixBaseSha -or $branch -cne "master") {
            Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_CONTEXT_INVALID pre_push"
        } else {
            $parentReference = $headParents[1]
        }
    }

    $nameStatusLines = if ($CurrentPhase -ceq "pre_commit") {
    ...
    $nameStatusExitCode = $LASTEXITCODE
    $nameStatusFiles = [System.Collections.Generic.List[string]]::new()
    $isValidNameStatus = $nameStatusExitCode -eq 0
    ...
    if (-not $isValidNameStatus -or -not (Test-P1F0143SpecApprovalTransitionHotfixFileSet -Files $nameStatusFiles)) {
        Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_FILE_SET_INVALID"
    }

    if ($CurrentTaskId -cne $p1F0143SpecApprovalTransitionHotfixParentTaskId -or $queueTaskId -cne $p1F0143SpecApprovalTransitionHotfixParentTaskId -or $taskStatus -cne "in_progress") {
        Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_CONTEXT_INVALID task"
    }

    $materializedAuthorizationPath = ((& git -C $Root ls-tree -r --name-only $parentReference -- $p1F0143SpecApprovalTransitionHotfixAuthorizationPath) -join "").Trim()
    if ($LASTEXITCODE -ne 0) {
        Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_PARENT_INSPECTION_FAILED"
    } elseif ($materializedAuthorizationPath -ceq $p1F0143SpecApprovalTransitionHotfixAuthorizationPath) {
        Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_ALREADY_MATERIALIZED"
    }

    $contentSnapshot = if ($CurrentPhase -ceq "pre_commit") { "INDEX" } else { "HEAD" }
    $authorizationText = Get-P1F0143SpecApprovalTransitionHotfixSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0143SpecApprovalTransitionHotfixAuthorizationPath
    foreach ($fieldContract in @(
        ...
        if ($fieldMatches.Count -ne 1 -or -not [regex]::IsMatch($fieldMatches[0].Value, $fieldContract.Expected)) {
            Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_AUTHORIZATION_INVALID"
            break
        }
    }
    ...
    if (($authorizationFiles -join "|") -cne ($expectedAuthorizationFiles -join "|")) {
        Add-Finding "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_AUTHORIZATION_FILE_SET_INVALID"
    }
    ...
    foreach ($projection in @(
        ...
        $parentText = (Get-GitFileText -Root $Root -Reference $parentReference -Path $projection.Path) -replace "`r`n?", "`n"
        $currentText = (Get-P1F0143SpecApprovalTransitionHotfixSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $projection.Path) -replace "`r`n?", "`n"
        $expectedText = $parentText
        $validProjection = $true
        foreach ($replacement in $projection.Replacements) {
            if ([regex]::Matches($expectedText, [regex]::Escape($replacement.Anchor)).Count -ne 1) { $validProjection = $false; break }
        ...
        if (-not $validProjection -or $currentText -cne $expectedText) {
            $projectionFinding = if ($projection.Label -eq "STATE") {
                "P1_PROGRAM_F0143_SPEC_APPROVAL_TRANSITION_HOTFIX_STATE_INVALID"
            } else {
    ...
    $evidenceText = Get-P1F0143SpecApprovalTransitionHotfixSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0143SpecApprovalTransitionHotfixEvidencePath
    $auditText = Get-P1F0143SpecApprovalTransitionHotfixSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0143SpecApprovalTransitionHotfixAuditPath
    Test-P1F0115ModulePrecommitHotfixReviewContract -EvidenceText $evidenceText -AuditText $auditText
...
function Get-P1MechanismBootstrapPreCommitCorrectionNameStatus {
    param([Parameter(Mandatory = $true)][string]$Root)

    $records = [System.Collections.Generic.List[object]]::new()
    $isValid = $true
    $lines = @(& git --no-optional-locks -C $Root diff --cached --name-status --no-renames --diff-filter=ACMRTD)
    if ($LASTEXITCODE -ne 0) { $isValid = $false }
    foreach ($line in $lines) {
        ...
        $records.Add([pscustomobject]@{
                ...
                Path = ConvertTo-NormalizedPath -Path $match.Groups[2].Value
            })
    }
    $expected = @($p1MechanismBootstrapPreCommitCorrectionFiles | ForEach-Object { ConvertTo-NormalizedPath -Path $_ })
    $actual = @($records | ForEach-Object { $_.Path })
    if ($records.Count -ne $expected.Count -or $actual.Count -ne @($actual | Sort-Object -CaseSensitive -Unique).Count) { $isValid = $false }
    $actualCanonical = @($actual | Sort-Object -CaseSensitive) -join "|"
    $expectedCanonical = @($expected | Sort-Object -CaseSensitive) -join "|"
    if ($actualCanonical -cne $expectedCanonical) { $isValid = $false }
    foreach ($record in $records) {
        if ($record.Status -cne "M") { $isValid = $false }
    }
    return [pscustomobject]@{ Valid = [bool]$isValid; Records = @($records); Files = @($actual) }
}

function Get-P1MechanismBootstrapCommittedManualNameStatus {
    param([Parameter(Mandatory = $true)][string]$Root)

    $records = [System.Collections.Generic.List[object]]::new()
    $files = [System.Collections.Generic.List[string]]::new()
    $isValid = $true
    $nameStatusLines = @(& git --no-optional-locks -C $Root diff --name-status --no-renames $p1MechanismBootstrapBaseSha HEAD)
    if ($LASTEXITCODE -ne 0) { $isValid = $false }
    foreach ($nameStatusLine in $nameStatusLines) {
        $match = [regex]::Match(([string]$nameStatusLine), '^([AM])	(.+)$')
        if (-not $match.Success) {
            $isValid = $false
            continue
        }
    ...
    $expectedStatusByPath = @{}
    foreach ($expectedPath in $p1MechanismBootstrapFiles) {
        $normalizedExpectedPath = ConvertTo-NormalizedPath -Path $expectedPath
        $baseMaterializedPath = ((& git -C $Root ls-tree --name-only $p1MechanismBootstrapBaseSha -- $normalizedExpectedPath) -join '').Trim()
        if ($LASTEXITCODE -ne 0) { $isValid = $false }
        $expectedStatusByPath[$normalizedExpectedPath] = if ($baseMaterializedPath -ceq $normalizedExpectedPath) { 'M' } else { 'A' }
    }
    if (-not (Test-P1MechanismBootstrapFileSet -Files @($files))) { $isValid = $false }
    if ($records.Count -ne $p1MechanismBootstrapFiles.Count) { $isValid = $false }
    foreach ($record in $records) {
        if (-not $expectedStatusByPath.ContainsKey($record.Path) -or $expectedStatusByPath[$record.Path] -cne $record.Status) { $isValid = $false }
    }
...
function Get-P1MechanismBootstrapCommittedManualContractTreeSha256 {
    param([Parameter(Mandatory = $true)][string]$Root)

    $nameStatus = Get-P1MechanismBootstrapCommittedManualNameStatus -Root $Root
    if (-not $nameStatus.Valid) { return '' }
    $records = [System.Collections.Generic.List[string]]::new()
    foreach ($path in ($p1MechanismBootstrapManualContractFiles | Sort-Object -CaseSensitive)) {
        ...
        if ($path -ceq 'scripts/agent-system/Test-P1RemediationSerialProgram.ps1') {
            $text = [regex]::Replace($text, '(?m)^\$p1MechanismBootstrapManualContractTreeSha256\s*=\s*"[^"]*"\s*$', '$p1MechanismBootstrapManualContractTreeSha256 = ""')
            $text = [regex]::Replace($text, '(?m)^\$p1MechanismBootstrapPreCommitCorrectionTreeSha256\s*=\s*"[^"]*"\s*$', '$p1MechanismBootstrapPreCommitCorrectionTreeSha256 = ""')
            $text = [regex]::Replace($text, '(?m)^\$p1MechanismBootstrapManualHashCorrectionTreeSha256\s*=\s*"[^"]*"\s*$', '$p1MechanismBootstrapManualHashCorrectionTreeSha256 = ""')
        }
        $sha256 = Get-P1MechanismBootstrapNormalizedTextSha256 -Text $text
        $status = @($nameStatus.Records | Where-Object { $_.Path -ceq $path } | Select-Object -ExpandProperty Status)
        if ($status.Count -ne 1) { return '' }
        $records.Add("$status`t$path`t$sha256")
    }
    return Get-P1MechanismBootstrapNormalizedTextSha256 -Text ("p1-mechanism-bootstrap-manual-contract-v1`ncount=$($records.Count)`n$($records -join "`n")`n")
}

function Get-P1MechanismBootstrapGitIndexText {
    param(
        [Parameter(Mandatory = $true)][string]$Root,
        [Parameter(Mandatory = $true)][string]$Path
    )
    ...
    try {
        if (-not $process.Start()) { return [pscustomobject]@{ Valid = $false; Text = "" } }
        $stderrTask = $process.StandardError.ReadToEndAsync()
        $process.StandardOutput.BaseStream.CopyTo($output)
        $process.WaitForExit()
        $stderrTask.GetAwaiter().GetResult() | Out-Null
        if ($process.ExitCode -ne 0) { return [pscustomobject]@{ Valid = $false; Text = "" } }
        try {
            $text = [System.Text.UTF8Encoding]::new($false, $true).GetString($output.ToArray())
        } catch {
...
function Get-P1MechanismBootstrapPreCommitCorrectionTreeSha256 {
    param([Parameter(Mandatory = $true)][string]$Root)

    $records = [System.Collections.Generic.List[string]]::new()
    foreach ($path in ($p1MechanismBootstrapManualContractFiles | Sort-Object -CaseSensitive)) {
        # Read staged blob bytes directly so native stdout decoding cannot vary by PowerShell runtime.
        $indexText = Get-P1MechanismBootstrapGitIndexText -Root $Root -Path $path
        if (-not $indexText.Valid) { return '' }
        $text = $indexText.Text
        if ($path -ceq 'scripts/agent-system/Test-P1RemediationSerialProgram.ps1') {
            $text = [regex]::Replace($text, '(?m)^\$p1MechanismBootstrapManualContractTreeSha256\s*=\s*"[^"]*"\s*$', '$p1MechanismBootstrapManualContractTreeSha256 = ""')
            $text = [regex]::Replace($text, '(?m)^\$p1MechanismBootstrapPreCommitCorrectionTreeSha256\s*=\s*"[^"]*"\s*$', '$p1MechanismBootstrapPreCommitCorrectionTreeSha256 = ""')
        }
        $baseMaterializedPath = ((& git -C $Root ls-tree --name-only $p1MechanismBootstrapBaseSha -- $path) -join '').Trim()
        if ($LASTEXITCODE -ne 0) { return '' }
        $status = if ($baseMaterializedPath -ceq $path) { 'M' } else { 'A' }
        $records.Add("$status`t$path`t$(Get-P1MechanismBootstrapNormalizedTextSha256 -Text $text)")
    }
    return Get-P1MechanismBootstrapNormalizedTextSha256 -Text ("p1-mechanism-bootstrap-manual-contract-v1`ncount=$($records.Count)`n$($records -join "`n")`n")
}

function Get-P1MechanismBootstrapManualHashCorrectionTreeSha256 {
    param([Parameter(Mandatory = $true)][string]$Root)
...
function Get-P1MechanismBootstrapManualHashCorrectionNameStatus {
    param([Parameter(Mandatory = $true)][string]$Root)
    ...
    foreach ($line in $lines) {
        $match = [regex]::Match(([string]$line), '^([AM])\t(.+)$')
        if (-not $match.Success) { $isValid = $false; continue }
        $records.Add([pscustomobject]@{ Status = $match.Groups[1].Value; Path = ConvertTo-NormalizedPath -Path $match.Groups[2].Value })
    }
    $expected = @($p1MechanismBootstrapManualHashCorrectionFiles | ForEach-Object { ConvertTo-NormalizedPath -Path $_ })
    $actual = @($records | ForEach-Object { $_.Path })
    if ($records.Count -ne $expected.Count -or $actual.Count -ne @($actual | Sort-Object -CaseSensitive -Unique).Count) { $isValid = $false }
    if ((@($actual | Sort-Object -CaseSensitive) -join '|') -cne (@($expected | Sort-Object -CaseSensitive) -join '|')) { $isValid = $false }
    foreach ($record in $records) { if ($record.Status -cne 'M') { $isValid = $false } }
    return [pscustomobject]@{ Valid = [bool]$isValid; Records = @($records); Files = @($actual) }
}
...
function Test-P1MechanismBootstrapManualHashCorrectionCandidate {
    ...
    $staged = Get-P1MechanismBootstrapManualHashCorrectionNameStatus -Root $Root
    if (-not $staged.Valid -or $CurrentTaskId -cne $p1MechanismBootstrapTaskId) { return $false }
    if ((Get-ScalarValue -Block $CurrentTaskBlock -Key 'status') -cne 'ready_for_closeout' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'taskKind') -cne 'mechanism_hardening' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'findingIds') -cne '[]' `
        -or @(Get-ListValues -Block $CurrentTaskBlock -Key 'findingIds').Count -ne 0 `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'productClosureContribution') -cne 'none') { return $false }
    $head = ((& git -C $Root rev-parse HEAD) -join '').Trim()
    $origin = ((& git -C $Root rev-parse origin/master) -join '').Trim()
    $branch = ((& git -C $Root branch --show-current) -join '').Trim()
    ...
    if (@(& git --no-optional-locks -C $Root diff --name-only --no-renames).Count -ne 0 -or @(& git --no-optional-locks -C $Root ls-files --others --exclude-standard).Count -ne 0) { return $false }
    $evidence = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapEvidencePath -replace '/', '\')) -Raw -Encoding UTF8
    $audit = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapAuditPath -replace '/', '\')) -Raw -Encoding UTF8
    foreach ($text in @($evidence, $audit)) {
        if ($text -notmatch '(?m)^\s*-\s+manualHashSelfIntegrityCorrection:\s*approved_one_time\s*$' `
            -or $text -notmatch ('(?m)^\s*-\s+manualHashSelfIntegrityBaseSha:\s*' + [regex]::Escape($p1MechanismBootstrapManualHashCorrectionBaseSha) + '\s*$') `
            -or $text -notmatch ('(?m)^\s*-\s+manualHashSelfIntegrityOriginMasterSha:\s*' + [regex]::Escape($p1MechanismBootstrapManualHashCorrectionOriginMasterSha) + '\s*$')) { return $false }
    }
    $scriptText = Get-Content -LiteralPath (Join-Path $Root 'scripts/agent-system/Test-P1RemediationSerialProgram.ps1') -Raw -Encoding UTF8
    $scriptHashMatches = $scriptText -match ('(?m)^\$p1MechanismBootstrapManualContractTreeSha256\s*=\s*"' + [regex]::Escape($p1MechanismBootstrapManualContractTreeSha256) + '"\s*$')
    if (-not $scriptHashMatches) { return $false }
    return (Get-P1MechanismBootstrapManualHashCorrectionTreeSha256 -Root $Root) -ceq $p1MechanismBootstrapManualHashCorrectionTreeSha256
}
...
function Test-P1MechanismBootstrapManualHashCorrectionCommittedCandidate {
    ...
    if ($CurrentTaskId -cne $p1MechanismBootstrapTaskId `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'status') -cne 'ready_for_closeout' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'taskKind') -cne 'mechanism_hardening' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'findingIds') -cne '[]' `
        -or @(Get-ListValues -Block $CurrentTaskBlock -Key 'findingIds').Count -ne 0 `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'productClosureContribution') -cne 'none') { return $false }
    $head = ((& git -C $Root rev-parse HEAD) -join '').Trim()
    $parents = @(((& git -C $Root rev-list --parents -n 1 HEAD) -join '').Trim() -split '\s+')
    $origin = ((& git -C $Root rev-parse origin/master) -join '').Trim()
    $branch = ((& git -C $Root branch --show-current) -join '').Trim()
    if ($parents.Count -ne 2 -or $parents[1] -cne $p1MechanismBootstrapManualHashCorrectionBaseSha `
        -or $origin -cne $p1MechanismBootstrapManualHashCorrectionOriginMasterSha `
        -or $branch -cne 'master' `
        -or @(& git --no-optional-locks -C $Root status --porcelain --untracked-files=all).Count -ne 0) { return $false }
    $records = @(& git --no-optional-locks -C $Root diff-tree --no-commit-id --name-status --no-renames -r HEAD)
    ...
    if ($actual.Count -ne $expected.Count -or (@($actual | Sort-Object -CaseSensitive -Unique).Count -ne $actual.Count) `
        -or ((@($actual | Sort-Object -CaseSensitive) -join '|') -cne ($expected -join '|'))) { return $false }
    $evidence = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapEvidencePath -replace '/', '\')) -Raw -Encoding UTF8
    $audit = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapAuditPath -replace '/', '\')) -Raw -Encoding UTF8
    if (-not (Test-P1MechanismBootstrapManualHashCorrectionReviewContract -Root $Root)) { return $false }
    foreach ($text in @($evidence, $audit)) {
        if ($text -notmatch '(?m)^\s*-\s+manualHashSelfIntegrityCorrection:\s*approved_one_time\s*$' `
            -or $text -notmatch ('(?m)^\s*-\s+manualHashSelfIntegrityBaseSha:\s*' + [regex]::Escape($p1MechanismBootstrapManualHashCorrectionBaseSha) + '\s*$') `
            -or $text -notmatch ('(?m)^\s*-\s+manualHashSelfIntegrityOriginMasterSha:\s*' + [regex]::Escape($p1MechanismBootstrapManualHashCorrectionOriginMasterSha) + '\s*$')) { return $false }
    }
    return (Get-P1MechanismBootstrapCommittedManualContractTreeSha256 -Root $Root) -ceq $p1MechanismBootstrapManualContractTreeSha256
}

function Test-P1MechanismBootstrapManualHashCorrectionReviewContract {
    param([Parameter(Mandatory = $true)][string]$Root)

    $evidence = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapEvidencePath -replace '/', '\')) -Raw -Encoding UTF8
    $audit = Get-Content -LiteralPath (Join-Path $Root ($p1MechanismBootstrapAuditPath -replace '/', '\')) -Raw -Encoding UTF8
    return $evidence -match '(?im)^###\s+2026-07-20 manual-hash self-integrity correction focused GREEN\s*$' `
        -and $evidence -match '(?im)^- Windows PowerShell 5\.1 and `pwsh` 7 P1 pre-commit both exited `0' `
        -and $audit -match '(?im)^###\s+2026-07-20 approved manual-hash self-integrity correction focused review\s*$' `
        -and $audit -match '(?im)^- Decision:\s*APPROVE_MANUAL_HASH_CORRECTION_SCOPE\s*$'
}

function Test-P1MechanismBootstrapPreCommitScopeCorrectionAnchors {
    param(
        [Parameter(Mandatory = $true)][string]$Root,
        [Parameter(Mandatory = $true)][string]$CurrentTaskId,
        [Parameter(Mandatory = $true)][AllowEmptyCollection()][AllowEmptyString()][string[]]$CurrentTaskBlock
    ...
    $originMasterSha = ((& git -C $Root rev-parse origin/master) -join '').Trim()
    $branch = ((& git -C $Root branch --show-current) -join '').Trim()
    $staged = Get-P1MechanismBootstrapPreCommitCorrectionNameStatus -Root $Root
    $unstaged = @(& git --no-optional-locks -C $Root diff --name-only --no-renames)
    $untracked = @(& git --no-optional-locks -C $Root ls-files --others --exclude-standard)
    if (-not $staged.Valid -or $headSha -cne $p1MechanismBootstrapPreCommitCorrectionBaseSha -or $originMasterSha -cne $headSha -or $branch -cne $p1MechanismBootstrapPreCommitCorrectionBranch -or $unstaged.Count -ne 0 -or $untracked.Count -ne 0) {
        Add-Finding 'P1_PROGRAM_MECHANISM_BOOTSTRAP_PRE_COMMIT_SCOPE_CORRECTION_INVALID'
    }
    if ($CurrentTaskId -cne $p1MechanismBootstrapTaskId `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'status') -cne 'ready_for_closeout' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'taskKind') -cne 'mechanism_hardening' `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'findingIds') -cne '[]' `
        -or @(Get-ListValues -Block $CurrentTaskBlock -Key 'findingIds').Count -ne 0 `
        -or (Get-ScalarValue -Block $CurrentTaskBlock -Key 'productClosureContribution') -cne 'none') {
        Add-Finding 'P1_PROGRAM_MECHANISM_BOOTSTRAP_PRE_COMMIT_SCOPE_CORRECTION_INVALID'
    }
    $stateText = Get-Content -LiteralPath (Join-Path $Root $ProjectStatePath) -Raw -Encoding UTF8
    $queueText = Get-Content -LiteralPath (Join-Path $Root $QueuePath) -Raw -Encoding UTF8
    if ((Get-P1MechanismBootstrapNormalizedTextSha256 -Text $stateText) -cne $p1MechanismBootstrapManualReadyStateProjectionSha256 -or (Get-P1MechanismBootstrapNormalizedTextSha256 -Text $queueText) -cne $p1MechanismBootstrapManualReadyQueueProjectionSha256) {
        Add-Finding 'P1_PROGRAM_MECHANISM_BOOTSTRAP_PRE_COMMIT_SCOPE_CORRECTION_INVALID'
    }
    $authorizationPath = Join-Path $Root $p1MechanismBootstrapAuthorizationPath
    $authorizationText = Get-Content -LiteralPath $authorizationPath -Raw -Encoding UTF8
    foreach ($pattern in @(
            '(?m)^Approval state:\s*approved\s*$',
            '(?m)^The current user approved one one-time, exact-allowlist pre-commit scope-correction channel',
            ('(?m)^The current user approved.*`codex/p1-mechanism-bootstrap-manual-recognition`'),
            ('(?m)^The current user approved.*`' + [regex]::Escape($p1MechanismBootstrapPreCommitCorrectionBaseSha) + '`')
        )) {
        if ($authorizationText -notmatch $pattern) { Add-Finding 'P1_PROGRAM_MECHANISM_BOOTSTRAP_PRE_COMMIT_SCOPE_CORRECTION_INVALID' }
    }
    ...
    if ((Get-P1MechanismBootstrapPreCommitCorrectionTreeSha256 -Root $Root) -cne $p1MechanismBootstrapPreCommitCorrectionTreeSha256) {
        Add-Finding 'P1_PROGRAM_MECHANISM_BOOTSTRAP_PRE_COMMIT_SCOPE_CORRECTION_INVALID'
    }
    if ($script:findings.Count -eq $findingCountBefore) { Write-Output 'p1MechanismBootstrapPreCommitScopeCorrection: approved_one_time' }
}
...
function Test-P1MechanismBootstrapAnchors {
    ...
    $branch = ((& git -C $Root branch --show-current) -join "").Trim()
    $parentReference = "HEAD"
    $isCommittedManualContract = $CurrentPhase -ceq "manual" -and (Get-ScalarValue -Block $CurrentTaskBlock -Key "status") -ceq "ready_for_closeout"
    if ($isCommittedManualContract) {
        $headParents = @(((& git -C $Root rev-list --parents -n 1 HEAD) -join "").Trim() -split '\s+')
        $originMasterSha = ((& git -C $Root rev-parse origin/master) -join "").Trim()
        $worktreeStatus = @(& git -C $Root status --porcelain --untracked-files=all)
        if ($headParents.Count -ne 2 -or $headParents[1] -cne $p1MechanismBootstrapManualContractParentSha `
            -or $originMasterSha -cne $headSha -or $branch -cne "master" -or $worktreeStatus.Count -ne 0) {
            Add-Finding "P1_PROGRAM_MECHANISM_BOOTSTRAP_MANUAL_CONTRACT_INVALID"
        }
        $parentReference = $p1MechanismBootstrapBaseSha
    } elseif ($CurrentPhase -ceq "manual") {
    ...
    $nameStatusLines = if ($isCommittedManualContract) {
        $manualNameStatus = Get-P1MechanismBootstrapCommittedManualNameStatus -Root $Root
    ...
    $expectedStateProjectionSha256 = if ($isCommittedManualContract) { $p1MechanismBootstrapManualReadyStateProjectionSha256 } elseif ($CurrentPhase -ceq "manual") { $p1MechanismBootstrapManualStateProjectionSha256 } else { $p1MechanismBootstrapStateProjectionSha256 }
    $expectedQueueProjectionSha256 = if ($isCommittedManualContract) { $p1MechanismBootstrapManualReadyQueueProjectionSha256 } elseif ($CurrentPhase -ceq "manual") { $p1MechanismBootstrapManualQueueProjectionSha256 } else { $p1MechanismBootstrapQueueProjectionSha256 }
    ...
    if ($isCommittedManualContract -and (Get-P1MechanismBootstrapCommittedManualContractTreeSha256 -Root $Root) -cne $p1MechanismBootstrapManualContractTreeSha256) {
        Add-Finding "P1_PROGRAM_MECHANISM_BOOTSTRAP_MANUAL_CONTRACT_INVALID"
    }
...
function Test-P1F0117SmokeScopeCloseoutLifecycleHotfixAnchors {
    ...
    if ($CurrentPhase -eq "pre_commit") {
        if ($headSha -ne $p1F0117SmokeScopeCloseoutLifecycleHotfixBaseSha -or $branch -ne $p1F0117SmokeScopeCloseoutLifecycleHotfixBranch) {
            Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_CONTEXT_INVALID pre_commit"
        }
        & git -C $Root diff --quiet
        $unstagedTrackedExitCode = $LASTEXITCODE
        $untrackedFiles = @(& git -C $Root ls-files --others --exclude-standard)
        if ($unstagedTrackedExitCode -ne 0 -or $LASTEXITCODE -ne 0 -or $untrackedFiles.Count -gt 0) {
            Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_PARTIAL_STAGE_INVALID"
        }
    } else {
        $headParents = @(((& git -C $Root rev-list --parents -n 1 HEAD) -join "").Trim() -split '\s+')
        $originMasterSha = ((& git -C $Root rev-parse origin/master) -join "").Trim()
        if ($headParents.Count -ne 2 -or $headParents[1] -ne $p1F0117SmokeScopeCloseoutLifecycleHotfixBaseSha -or $originMasterSha -ne $p1F0117SmokeScopeCloseoutLifecycleHotfixBaseSha -or $branch -ne "master") {
            Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_CONTEXT_INVALID pre_push"
        } else {
            $parentReference = $headParents[1]
        }
    }

    if ($CurrentTaskId -ne $p1F0117SmokeScopeCloseoutLifecycleHotfixParentTaskId -or $taskStatus -ne "in_progress") {
        Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_CONTEXT_INVALID task"
    }

    $materializedAuthorizationPath = ((& git -C $Root ls-tree -r --name-only $parentReference -- $p1F0117SmokeScopeCloseoutLifecycleHotfixAuthorizationPath) -join "").Trim()
    if ($LASTEXITCODE -ne 0) {
        Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_PARENT_INSPECTION_FAILED"
    } elseif ($materializedAuthorizationPath -eq $p1F0117SmokeScopeCloseoutLifecycleHotfixAuthorizationPath) {
        Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_ALREADY_MATERIALIZED"
    }

    $contentSnapshot = if ($CurrentPhase -eq "pre_commit") { "INDEX" } else { "HEAD" }
    $authorizationText = Get-GitSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0117SmokeScopeCloseoutLifecycleHotfixAuthorizationPath
    foreach ($fieldContract in @(
        @{ Key = 'Status'; Expected = '(?i)^Status:\s*approved\s*$' },
        ...
        $fieldMatches = @([regex]::Matches($authorizationText, "(?im)^$([regex]::Escape($fieldContract.Key))\s*:.*$"))
        if ($fieldMatches.Count -ne 1 -or $fieldMatches[0].Value -notmatch $fieldContract.Expected) {
            Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_AUTHORIZATION_INVALID"
            break
        }
    }
    ...
    if (($authorizationFiles -join "|") -cne ($expectedAuthorizationFiles -join "|")) {
        Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_AUTHORIZATION_FILE_SET_INVALID"
    }

    $reviewFindingCountBefore = $script:findings.Count
    $evidenceText = Get-GitSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0117SmokeScopeCloseoutLifecycleHotfixEvidencePath
    $auditText = Get-GitSnapshotFileText -Root $Root -Snapshot $contentSnapshot -Path $p1F0117SmokeScopeCloseoutLifecycleHotfixAuditPath
    Test-P1F0115ModulePrecommitHotfixReviewContract -EvidenceText $evidenceText -AuditText $auditText
    if ($script:findings.Count -gt $reviewFindingCountBefore) { Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_REVIEW_INVALID" }
...
$isP1MechanismBootstrapManualExistingContract = $false
...
# C6-MANUAL-MECHANISM-BOOTSTRAP-BEGIN
    ...
    if ($isP1MechanismBootstrapManualCandidate) {
        ...
        $manualTaskStatus = Get-ScalarValue -Block $taskBlock -Key "status"
        ...
        $isP1MechanismBootstrapManualExistingContract = $manualTaskStatus -ceq "ready_for_closeout"
        $isP1MechanismBootstrapManualContextValid = (Get-ScalarValue -Block $taskBlock -Key "taskKind") -ceq "mechanism_hardening" `
            ...
            -and (($isP1MechanismBootstrapManualExistingContract -and $manualTaskStatus -ceq "ready_for_closeout") -or (-not $isP1MechanismBootstrapManualExistingContract -and -not [string]::IsNullOrWhiteSpace($manualCanonicalTaskWorktreePath) -and $manualCanonicalTaskWorktreePath -ieq $manualCanonicalRepositoryRoot)) `
        ...
        if ($isP1MechanismBootstrapManualExistingContract) {
            $manualNameStatus = Get-P1MechanismBootstrapCommittedManualNameStatus -Root $RepositoryRoot
...
$isP1MechanismBootstrapPreCommitScopeCorrectionCandidate = $Phase -ceq 'pre_commit' `
    -and $stateCurrentTaskId -ceq $p1MechanismBootstrapTaskId `
    -and (Get-ScalarValue -Block $taskBlock -Key 'status') -ceq 'ready_for_closeout' `
    -and (Get-P1MechanismBootstrapPreCommitCorrectionNameStatus -Root $RepositoryRoot).Valid
# C6-PRE-COMMIT-MECHANISM-SCOPE-CORRECTION-END
$isP1MechanismBootstrapManualHashCorrectionCandidate = if ($Phase -ceq 'pre_commit') {
    Test-P1MechanismBootstrapManualHashCorrectionCandidate -Root $RepositoryRoot -CurrentTaskId $stateCurrentTaskId -CurrentTaskBlock $taskBlock
} elseif ($Phase -ceq 'pre_push') {
    Test-P1MechanismBootstrapManualHashCorrectionCommittedCandidate -Root $RepositoryRoot -CurrentTaskId $stateCurrentTaskId -CurrentTaskBlock $taskBlock
} else { $false }
...
if ($Phase -in @("pre_commit", "pre_push") -and -not $SkipGitChecks -and -not $p1ApprovedSameTaskTransitionIsExactBootstrap -and -not $isP1MechanismBootstrapPreCommitScopeCorrectionCandidate -and -not $isP1MechanismBootstrapManualHashCorrectionCandidate) {
...
$isP1F0117SmokeScopeCloseoutLifecycleHotfixCandidate = $Phase -in @("pre_commit", "pre_push") `
    -and @($normalizedFilesToCheck | Where-Object { $_ -in $p1F0117SmokeScopeCloseoutLifecycleHotfixIdentityFiles }).Count -gt 0
$isP1F0117SmokeScopeCloseoutLifecycleHotfixScope = $isP1F0117SmokeScopeCloseoutLifecycleHotfixCandidate -and (Test-P1F0117SmokeScopeCloseoutLifecycleHotfixFileSet -Files $filesToCheck)
...
$isP1F0143SpecApprovalTransitionHotfixCandidate = @("pre_commit", "pre_push") -ccontains $Phase `
...
$isP1MechanismBootstrapCandidate = $isP1MechanismBootstrapStageCandidate -or $isP1MechanismBootstrapManualCandidate -or $isP1MechanismBootstrapPreCommitScopeCorrectionCandidate -or $isP1MechanismBootstrapManualHashCorrectionCandidate
$isP1MechanismBootstrapScope = if ($isP1MechanismBootstrapManualCandidate) {
    $isP1MechanismBootstrapManualContextValid `
        ...
        -and ($isP1MechanismBootstrapManualExistingContract -or (Test-P1MechanismBootstrapFileSet -Files $filesToCheck))
} elseif ($isP1MechanismBootstrapPreCommitScopeCorrectionCandidate) {
    $true
} elseif ($isP1MechanismBootstrapManualHashCorrectionCandidate) {
    $true
} else {
...
if ($isP1F0117SmokeScopeCloseoutLifecycleHotfixCandidate -and -not $isP1F0117SmokeScopeCloseoutLifecycleHotfixScope) {
    Add-Finding "P1_PROGRAM_F0117_SMOKE_SCOPE_CLOSEOUT_LIFECYCLE_HOTFIX_ALLOWLIST_MISMATCH"
}
...
if ($isP1MechanismBootstrapManualHashCorrectionCandidate) {
    Write-Output 'p1MechanismBootstrapManualHashCorrection: approved_one_time'
} elseif ($isP1MechanismBootstrapPreCommitScopeCorrectionCandidate) {
    Test-P1MechanismBootstrapPreCommitScopeCorrectionAnchors -Root $RepositoryRoot -CurrentTaskId $stateCurrentTaskId -CurrentTaskBlock $taskBlock
} elseif ($isP1MechanismBootstrapCandidate) {
...
if ($isP1F0117SmokeScopeCloseoutLifecycleHotfixScope) {
    Test-P1F0117SmokeScopeCloseoutLifecycleHotfixAnchors -Root $RepositoryRoot -CurrentTaskId $stateCurrentTaskId -CurrentTaskBlock $taskBlock -CurrentPhase $Phase
}
...
if ($taskBlock.Count -gt 0) {
    ...
    if ($isP1MechanismBootstrapManualHashCorrectionCandidate) {
        if (-not (Test-P1MechanismBootstrapManualHashCorrectionReviewContract -Root $RepositoryRoot)) {
            Add-Finding "P1_PROGRAM_MANUAL_HASH_CORRECTION_REVIEW_INVALID $stateCurrentTaskId"
        }
    } elseif ($requiresFinalReview) {
...
if ($isP1MechanismBootstrapManualHashCorrectionCandidate) { Write-Output "p1MechanismBootstrapManualHashCorrection: approved_one_time" }
