...
try {
    ...
    $primaryRoot = Join-Path -Path $automationRoot -ChildPath "tiku-module-run-v2-autopilot-2"
    New-Item -ItemType Directory -Path $primaryRoot -Force | Out-Null
    New-Item -ItemType Directory -Path $onDemandRoot -Force | Out-Null

    Write-ProjectState -Path $projectStatePath -AutomationId "tiku-module-run-v2-autopilot-2" -Status "ACTIVE"
    ...
    Write-AutomationToml -Path (Join-Path -Path $primaryRoot -ChildPath "automation.toml") -AutomationId "tiku-module-run-v2-autopilot-2" -Status "PAUSED"
