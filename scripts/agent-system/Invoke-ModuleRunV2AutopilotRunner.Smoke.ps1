...
"@ | Set-Content -LiteralPath $seedProposalQueuePath -Encoding UTF8
    ...
    Assert-Contains -Output $seedProposalOutput -Pattern "stopCardDecision: manual_required"
    Assert-Contains -Output $seedProposalOutput -Pattern "canAutoRecover: false"
    Assert-Contains -Output $seedProposalOutput -Pattern "blockerClass: approval_missing"
    Assert-Contains -Output $seedProposalOutput -Pattern "statePolicy: no_write_accounted"
...
"@ | Set-Content -LiteralPath $pendingSeedDecisionQueuePath -Encoding UTF8
    ...
    Assert-Contains -Output $pendingSeedDecisionOutput -Pattern "stopCardDecision: manual_required"
    Assert-Contains -Output $pendingSeedDecisionOutput -Pattern "canAutoRecover: false"
    Assert-Contains -Output $pendingSeedDecisionOutput -Pattern "blockerClass: approval_missing"
    Assert-Contains -Output $pendingSeedDecisionOutput -Pattern "statePolicy: no_write_accounted"
