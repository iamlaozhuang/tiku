...
function createOutputContractInstruction(
  ...
  return `输出 JSON；仅输出一个 JSON 对象，不要输出 Markdown 代码块、解释文字、编号列表或表格；必须包含 title、targetQuestionCount、sections、knowledgeCoverage、difficultyGoal、sourcePreference、questionTypeDistribution 和 paperStructure；targetQuestionCount 必须等于 ${requestedQuestionCount}；sourcePreference 必须为 ${generationParameters?.sourcePreference ?? "balanced"}；questionTypeDistribution 必须为 ${generationParameters?.questionTypeDistribution ?? "weak_point_priority"}，它只是可编辑建议，不是七题型强制比例；paperStructure 必须为 ${generationParameters?.paperStructure ?? "by_question_type"}；sections 每项必须包含 sectionKey、title、questionType、targetQuestionCount、targetScore、knowledgeNodeLabels 和 difficulty；questionType 只能为 single_choice、multi_choice、true_false、fill_blank、short_answer、case_analysis、calculation 之一；只生成组卷方案，不得生成或引用最终题目正文、选项、答案、解析、供应商请求载荷、原始提示词、内部日志或资料原文。`;
}
