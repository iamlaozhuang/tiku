    ...
    async listAiCallLogs(query: AiCallLogFactQuery) {
      ...
      const aiCallLogs = legacyResult.aiCallLogs.map<AiCallLogRecord>(
        (aiCallLog) => ({
          ...
          observationSchemaVersion: aiCallLog.observationSchemaVersion,
          tokenCountSource: aiCallLog.tokenCountSource,
          tokenEstimationMethod: aiCallLog.tokenEstimationMethod,
          latencySource: aiCallLog.latencySource,
