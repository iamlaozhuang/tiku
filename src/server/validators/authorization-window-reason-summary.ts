import type { AuthorizationWindowReasonSummaryInput } from "../models/authorization-window-reason-summary";

export type AuthorizationWindowReasonSummaryValidationResult =
  | {
      success: true;
      value: AuthorizationWindowReasonSummaryInput;
    }
  | {
      success: false;
      message: string;
...
const INVALID_AUTHORIZATION_WINDOW_REASON_SUMMARY_INPUT_MESSAGE =
  "Invalid authorization window reason summary input.";
...
export function normalizeAuthorizationWindowReasonSummaryInput(
  input: unknown,
): AuthorizationWindowReasonSummaryValidationResult {
  if (!isRecord(input)) {
    return {
      success: false,
      message: INVALID_AUTHORIZATION_WINDOW_REASON_SUMMARY_INPUT_MESSAGE,
    };
  }
