...
export type PersonalAiGenerationResultReferenceValidationResult =
  | {
      success: true;
      value: PersonalAiGenerationResultReferenceInput;
    }
  | {
      success: false;
      message: string;
...
const INVALID_PERSONAL_AI_GENERATION_RESULT_REFERENCE_INPUT_MESSAGE =
  "Invalid personal AI generation result reference input.";
...
export function normalizePersonalAiGenerationResultReferenceInput(
  input: unknown,
): PersonalAiGenerationResultReferenceValidationResult {
  if (!isRecord(input)) {
    return {
      success: false,
      message: INVALID_PERSONAL_AI_GENERATION_RESULT_REFERENCE_INPUT_MESSAGE,
    };
  }
