...
The current repository runtime has evolved since that selection. The implementation now uses Next.js 16 and React 19. As of the 2026-06-21 recheck, the AI SDK packages are present in `package.json`; RAG text splitting and Markdown/math rendering packages remain absent.
...
- installed but gated capabilities that still require Provider, env, data, or runtime approval before use.

This ADR records current reality only. It does not add, remove, upgrade, or downgrade any package.

## Current Package Baseline

As of 2026-06-21, `package.json` records this local runtime baseline:
...
| AI SDK packages         | `ai` ^6.0.204, `@ai-sdk/alibaba` ^1.0.28, `@ai-sdk/openai-compatible` ^2.0.50                                                          |
...
## Installed But Gated Capability Baseline

The installed AI SDK packages are a local dependency fact only. They do not approve:

- real Provider or model calls;
- Provider configuration or fallback-chain changes;
- `.env.local`, `.env.example`, secret, token, or API key reads or writes;
- prompt, provider payload, raw generated AI content, or raw answer evidence exposure;
- production, staging, cloud, payment, external-service, or Cost Calibration Gate work.

Any task that uses the installed AI SDK packages for real execution must still carry explicit queued scope and the relevant Provider/env/data/runtime approvals.
...
| --------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Optional AI providers       | Provider packages beyond the installed baseline                        | Deferred until an approved provider integration task passes dependency introduction gate and provider/env gates.              |
...
- installed AI SDK package baseline `ai` 6.x, `@ai-sdk/alibaba` 1.x, and `@ai-sdk/openai-compatible` 2.x;
...
Future tasks must treat absent ADR-001 packages as deferred, not as silently missing implementation. Future tasks must also treat installed AI SDK packages as dependency availability only, not as Provider/runtime approval. Introducing any deferred package requires:
...
- AI SDK package availability can be cited for architecture freshness, but not for live Provider readiness.
