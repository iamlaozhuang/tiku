# 2026-07-05 Stage C-1 Provider Freshness Bounded Smoke Rerun
...
| Secret source      | runtime process env label `ALIBABA_API_KEY` only     |
...
- Missing runtime secret source label.
- Provider target/model/host differs from the approved labels.
...
| Runtime secret source label present if calling | true or stop before calling | block  |
...
| Closeout state         | closeout gates passed after stop condition |
