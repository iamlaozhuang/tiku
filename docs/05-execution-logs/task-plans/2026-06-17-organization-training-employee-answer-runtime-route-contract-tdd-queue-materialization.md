...
- Task kind: docs/state queue alignment
- User approval: current prompt approves materializing
  `organization-training-employee-answer-runtime-route-contract-tdd` as the next
  ready-set item. The prompt does not approve source implementation in this
  task.
...
`organization-training-employee-answer-runtime-route-contract-tdd` after the
metadata-only repository contract is closed and before the schema migration
...
1. Add this closed docs/state materialization task entry.
2. Add a pending implementation task entry for
   `organization-training-employee-answer-runtime-route-contract-tdd`.
3. Put the new pending route task before
   `organization-training-draft-source-context-schema-migration` in queue order
   so the ready-set can select the employee answer route closure next.
4. Update project-state and coverage matrix to point at the materialized route
   task as the next local unit task.
5. Validate with docs/state diagnostics, scoped formatting, lint, typecheck,
   diff check, and Module Run v2 readiness gates.
