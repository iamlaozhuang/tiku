# Batch 222 Organization Training Context Plan

## Task

- Task id: `batch-222-organization-training-paper-and-mock-exam-context-usage-without-ex`
- Branch: `codex/batch-222-organization-training-context`
- Scope: low-risk local Module Run v2 L6 validation for paper and mock_exam context usage without exposing full paper content in evidence.
...
- Read batch-222 queue block and organization-training auto-seed evidence.
- Read existing organization-training service, route, and validator focused unit tests.
- Pre-edit auto-seed readiness passed for candidate batch-222.

## Implementation Strategy

1. Claim batch-222 on the current branch while preserving high-risk blocks.
2. Replace the advisory focused placeholder with the scoped unit command:
   `npm.cmd run test:unit -- src/server/services/organization-training-service.test.ts src/server/services/organization-training-route.test.ts src/server/validators/organization-training.test.ts`.
3. Run the existing focused unit coverage first. If it validates paper/mock_exam context metadata-only behavior, route contracts, and validator normalization, do not change source or tests.
...
5. Write redacted evidence and audit records without full paper content, raw employee answer text, provider payloads, internal DB rows, secrets, raw prompts, or raw generated AI content.
...
- `git diff --check`
- `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2PreCommitHardening.ps1 -TaskId batch-222-organization-training-paper-and-mock-exam-context-usage-without-ex`
- `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2ModuleCloseoutReadiness.ps1 -TaskId batch-222-organization-training-paper-and-mock-exam-context-usage-without-ex`
