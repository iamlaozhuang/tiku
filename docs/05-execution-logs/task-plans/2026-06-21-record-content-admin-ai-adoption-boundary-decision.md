# Task Plan: Record Content Admin AI Adoption Boundary Decision

## Scope

- Task id: `record-content-admin-ai-adoption-boundary-decision`.
- Branch: `codex/content-admin-ai-adoption-boundary-decision`.
- User decision: option A, use two-step adoption from isolated AI result to editable formal draft, then existing
...
1. Register the follow-up adoption boundary decision in `project-state.yaml` and `task-queue.yaml`.
2. Extend the content_admin AI policy document with the option A two-step adoption rule.
...
- No source, tests, schema, migration, seed, database, model output persistence, Provider call, prompt/provider payload,
  ...
  Gate work.
- This records the adoption boundary decision only; implementation remains blocked until fresh approval.
