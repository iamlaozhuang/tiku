...
## Narrow manual-contract recognition correction (2026-07-20)

- Authorization: current user approved one narrow correction: manual phase may recognize the already-materialized findingless mechanism-bootstrap contract and its smoke coverage only. This does not authorize changes to pre-commit/pre-push routing, ordinary SHA-drift handling, transition topology, approval, evidence, schema, product, dependency, runtime, database, Provider, P2, PR, force-push or deploy behavior.
- Root cause: after the mechanism implementation was merged and the state-only `in_progress -> ready_for_closeout` transition was pushed, the manual adapter still treated the task as an uncommitted bootstrap candidate. A clean `master` therefore exposed an empty worktree name-status and the old base/branch context, so manual could not recognize the existing contract.
- Exact correction boundary: manual-only recognition requires the exact task id/kind, literal `findingIds: []`, `productClosureContribution: none`, ready-for-closeout state projection, clean `master`, exact base-to-HEAD A/M file set, the fixed manual state/queue projections, the existing authorization/plan/evidence/audit contracts, and a bound manual contract tree hash. Any mismatch remains fail-closed. Pre-commit and pre-push code paths remain unchanged.
- Allowed files: this existing mechanism task's already-frozen 18 paths, with production edits restricted to `scripts/agent-system/Test-P1RemediationSerialProgram.ps1` and `scripts/agent-system/Test-P1RemediationSerialProgram.Smoke.ps1`; plan/evidence/audit/authorization are governance records only.
- Blocked files/actions: all product, schema/migration, database, dependency, Provider/runtime/browser, P2, secret/env, PR, force-push and deploy paths; no hook bypass or ordinary SHA-drift/topology relaxation.
- TDD RED: add a smoke case that runs manual against the committed ready-for-closeout mechanism contract and expects the current guard to fail with the mechanism context/file-set or legacy manual-review findings; retain adversarial wrong-state/wrong-tree negatives.
- GREEN: implement only the manual contract recognition branch and its smoke assertions; verify the committed `master` manual command passes in `standard` mode without ancestor permission, while ordinary drift/topology/authorization/evidence negatives still fail. No full matrix rerun is implied unless the affected smoke or manual contract changes require it.
- Stop condition: stop immediately if the correction requires changing pre-commit/pre-push semantics, widening the exact file set, weakening ordinary SHA drift/topology/approval/evidence gates, changing state/queue beyond the existing ready-for-closeout contract, or adding another finding-specific exception.
- Closeout entry: after targeted RED/GREEN, main-thread adversarial review, evidence/audit update and the existing mechanism closeout policy, return to the mechanism task's pending C7 closeout; this correction is governance-only and contributes no product finding closure.

## Self-Review

## One-time pre-commit scope-correction channel (2026-07-20)

- Authorization: current user approved one one-time, exact-allowlist pre-commit scope-correction channel for the already-materialized `ready_for_closeout` mechanism contract. This is a commit-routing compatibility correction only; it does not authorize ordinary SHA drift, transition topology, ancestor policy, approval/evidence weakening, hook bypass, product closure or any schema/database/dependency/provider/runtime/P2/PR/force-push/deploy work.
- Exact candidate: current task `p1-mechanism-execution-compatibility-v2-1-2026-07-19`, `taskKind: mechanism_hardening`, literal `findingIds: []`, `productClosureContribution: none`, HEAD/parent `d0b71842657f8f4df7e72d5fa6514b94d20b2de4`, branch `codex/p1-mechanism-bootstrap-manual-recognition`, origin/master equal to HEAD, and no unstaged or untracked files.
- Exact staged allowlist (all `M`, no other status/path): the four existing mechanism authorization/plan/evidence/audit records, `scripts/agent-system/Test-P1RemediationSerialProgram.ps1`, `scripts/agent-system/Test-P1RemediationSerialProgram.Smoke.ps1`, `scripts/agent-system/Test-ModuleRunV2PreCommitHardening.ps1`, and `scripts/agent-system/Test-ModuleRunV2PreCommitHardening.Smoke.ps1`. State/queue, pre-push, product and unrelated governance files are excluded.
- Required checks: exact staged name-status and count, fixed parent/branch/origin topology, ready state/queue projections, existing external authorization and review/evidence contracts, no generic transition fallback, no ordinary drift exception, and standard manual mode. A failed check emits a dedicated scope-correction finding and remains hard-blocked.
- RED/GREEN: add P1 and Module pre-commit smoke coverage for the exact positive candidate plus wrong file, unstaged drift, wrong branch, wrong parent and malformed authorization negatives. Run focused smoke first, then serial P1 smoke, Module pre-commit smoke, parser, diff check and the normal pre-commit hook. No pre-push semantics change.
- Stop: stop immediately if the channel requires a broader allowlist, a state/queue mutation, a hook bypass, a transition-only/ancestor grant, or any ordinary drift/topology/approval/evidence relaxation.
- Final normalized correction tree hash after the ordinal status-path uniqueness and cross-runtime raw-byte review: `9b5c22c2d2b3e06114a35181fb7cb9b55e33d26becca3b6e15fb681cf6c289ad` (the earlier `869027...`, `976911...`, `cbe21...` and `4a74...` candidates are stale and not admissible).
