# Security API Contract Input Validation Inventory Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to execute this plan task-by-task. Steps
> use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce a source-read-only inventory of REST API contract and input-validation risks, then split actionable
findings into future scoped fix tasks without changing source, tests, DB, Provider configuration, dependencies, or
release state.

**Architecture:** The task follows ADR-002 by tracing route handlers, validators, contracts, mappers, and relevant unit
coverage as read-only surfaces. Findings are recorded as redacted route/contract/validator risk rows, not raw request
payloads, database rows, credentials, or complete business content.

**Tech Stack:** Next.js route handlers, TypeScript validators/contracts/mappers, Markdown/YAML governance artifacts,
...
- Status: closed
- Planned at: `2026-06-29T08:55:08-07:00`
...
- [x] **Step 1: Read mandatory governance and latest predecessor evidence**
  ...
  Confirm current `master` and `origin/master` are aligned at `208471707`.

- [x] **Step 2: Create short branch**

  Run: `git switch -c codex/security-api-validation-inventory-20260629`
...
- [x] **Step 4: Build read-only API contract surface index**

  Use `rg --files` over read-only route, validator, contract, mapper, service, and unit-test surfaces. Record only file
  paths and count summaries.

- [x] **Step 5: Inspect API contract and input-validation paths**

  Read selected route handlers, validators, contracts, mappers, services, and tests. Classify risks around standard
  response envelopes, input validation, API naming, null/list behavior, pagination, public-id URL exposure, and error
  mapping. Record redacted summaries only.
...
```powershell
npx.cmd prettier --write --ignore-unknown docs/04-agent-system/state/project-state.yaml docs/04-agent-system/state/task-queue.yaml docs/01-requirements/traceability/2026-06-29-security-api-contract-input-validation-inventory.md docs/05-execution-logs/task-plans/2026-06-29-security-api-contract-input-validation-inventory.md docs/05-execution-logs/evidence/2026-06-29-security-api-contract-input-validation-inventory.md docs/05-execution-logs/audits-reviews/2026-06-29-security-api-contract-input-validation-inventory.md docs/05-execution-logs/acceptance/2026-06-29-security-api-contract-input-validation-inventory.md
...
git diff --check
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2PreCommitHardening.ps1 -TaskId security-api-contract-input-validation-inventory-2026-06-29
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2ModuleCloseoutReadiness.ps1 -TaskId security-api-contract-input-validation-inventory-2026-06-29
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2PrePushReadiness.ps1 -TaskId security-api-contract-input-validation-inventory-2026-06-29 -SkipRemoteAheadCheck
```
