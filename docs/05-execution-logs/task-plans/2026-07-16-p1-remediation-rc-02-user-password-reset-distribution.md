...
- runtime 不再解析用户 reset body，权限与 runtime readiness 通过后才调用共享高熵生成器；成功返回既有一次性 DTO 和 `no-store`，失败路径不返回 secret。
- UI 完全移除用户密码输入和 request body，用户/后台账号共享单一内存 secret；新 reset、dismiss 与刷新清除旧值。
- 首轮独立审查发现双击并发和网络异常 blocker；最终以同步 ref 锁、pending 禁用/入口拒绝、request sequence 与 finally 释放收敛，同一页面实例只允许一个 reset POST。
- 聚焦 41/41、完整 unit 2439/2439 及静态质量门禁通过；两项独立只读复核最终均为 `APPROVE`。
- 不声明跨标签页/客户端串行或密码写入与 session revoke 原子性；F-0030 与 RV-0016 保持独立开放边界。
