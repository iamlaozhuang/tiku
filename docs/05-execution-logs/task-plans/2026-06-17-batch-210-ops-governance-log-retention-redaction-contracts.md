# Task Plan: batch-210 ops-governance log retention redaction contracts
...
Implement a local service-contract read model for operations-facing `audit_log` and `ai_call_log` retention and redaction contracts. The read model must expose policy/status only: retention days, redaction state, reference existence state, blocked capabilities, and evidence policy.
...
1. Add focused unit tests for the desired ops-governance log retention/redaction contract.
