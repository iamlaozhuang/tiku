...
- Branch merged into `master` with `git merge --no-ff --no-commit codex/advanced-edition-cost-calibration-gate`.
- `git diff --check`: pass, no whitespace errors.
- `node .\node_modules\prettier\bin\prettier.cjs --check ...`: pass, all matched files use Prettier code style.
- `Select-String ... 'phase-30-advanced-edition-cost-calibration-gate', ... 'provider_cost_measurement'`: pass, blocked gate queue anchors confirmed in the merged `master` state.
