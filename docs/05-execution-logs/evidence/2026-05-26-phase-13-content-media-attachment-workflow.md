...
**Feature commit:** `331e21d fix(content): use managed media references`
...
**Actual path:** `/login` -> `/content/questions` -> verify locked-row affordance when present -> `新建题目` -> insert managed image reference from the browser-visible form -> `/content/materials` -> `/content/papers`.
...
- Actual: focused `content-action-closures` e2e passed and full e2e passed with 15 Chromium tests after the post-merge e2e selector was corrected to use the deterministic `新建题目` authoring form.
...
- Post-merge e2e initially failed because the local role-based fixture showed only locked accepted questions on the first content question page. The test was adjusted to verify the helper through the deterministic `新建题目` path while still checking the locked-row affordance when visible.
...
- `npm.cmd run format:check` initially hit sandbox `EPERM` while opening the local Prettier binary under `node_modules`; the same command passed after approved sandbox escalation.
...
- Merged `codex/phase-13-content-media-attachment-workflow` into `master` with `--no-ff`.
- Post-merge `npm.cmd run test:unit` passed: 130 files, 521 tests.
- Post-merge `npm.cmd run test:e2e` passed: 15 Chromium tests after the deterministic new-question path fix.
- Post-merge `npm.cmd run build` passed.
- Post-merge `npm.cmd run lint` passed.
- Post-merge `npm.cmd run typecheck` passed.
- Post-merge readiness, naming, git readiness, `git diff --check`, and `format:check` passed.
