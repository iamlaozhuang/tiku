...
- Merge target: `master`
- Source branch: `codex/phase-12-role-scenario-script-plan`
- Feature commit: `eaec8c6 docs(agent): add role scenario experience scripts`
- Initial merge commit before evidence amend: `bf95e0a merge: add phase 12 role scenario scripts`
- Master verification:
...
- Post-merge forbidden scope check:
  - No runtime/UI/test/package/lockfile/schema/migration/script changes.
  - No `.env.local` or `.env.example` read/write.
  - No provider/cloud/staging/prod/deploy access.
  - No secret/env value, generated redeem_code plaintext, raw provider payload, raw prompt, raw answer, or raw model response recorded in evidence.
