...
| `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\agent-system\Test-ModuleRunV2ModuleCloseoutReadiness.ps1 -TaskId batch-101-authorization-and-access-authorization-read-model-and-display-contrac`                                                                                          | pending commit evidence | Fresh owner-recovery run blocked only on `HARD_BLOCK_MISSING_BATCH_COMMIT_EVIDENCE`; this is expected before the first local task commit. |
...
- `git diff --check` pass.
- `Test-ModuleRunV2ModuleCloseoutReadiness.ps1 ...` blocked only on missing batch commit evidence before the first local task commit.
