...
- Branch merged into `master` with `git merge --no-ff --no-commit codex/advanced-edition-default-value-governance`.
- `git diff --check`: pass, no whitespace errors.
- `node .\node_modules\prettier\bin\prettier.cjs --check ...`: pass, all matched files use Prettier code style.
- `Select-String ... 'Acceptance Scenarios', ... 'production_enablement_blocked'`: pass, MVP and configuration governance anchors confirmed in the merged `master` state.
- `rg -n "license|exam_paper|exam paper|licence" ...`: pass, no matches in reviewed specs.
