...
2026-07-02 supersession note: older wording below that says organization admins "manage employees" is narrowed by
`CT-REQ-010`, `CT-REQ-043`, and `CT-REQ-054`. In the first release, employee import, mutation, transfer, disable,
unbind, and password reset remain platform `ops_admin` / `super_admin` operations. Organization admins see scoped
roster/status and authorization/status only unless a later task explicitly delegates a write flow.
...
| `org_standard_admin`        | fail                   | Dedicated organization admin workspace; employee roster/status and authorization/status view only; no training or AI generation.                   |
| `org_advanced_admin`        | fail                   | Dedicated organization admin workspace; employee roster/status and authorization/status view, enterprise training, and organization AI generation. |
...
| R3       | `modules/04-organization-training.md`, `stories/epic-02-organization-training.md`                  | `org_standard_admin` can view scoped employee roster/status and organization authorization/status only.                               | No employee mutation, enterprise training management, or AI generation for standard organization admin.                                        |
| R4       | `modules/04-organization-training.md`, `modules/08-organization-ai-generation.md`, related stories | `org_advanced_admin` can view scoped employee roster/status and manage training plus organization-owned AI question/paper generation. | Organization advanced backend entry must be discoverable, not URL-only.                                                                        |
